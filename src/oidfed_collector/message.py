...
from cryptojwt.utils import re
...
from idpyoidc.message import oidc, oauth2
...
def get_payload(self_signed_statement):
    """Imported from fedservice.entity.function"""
...
class WrongSubject(Exception):
    pass


class UnknownCriticalExtension(Exception):
    pass
...
class TrustMarkIssuer(Message):
    c_param = {"federation_status_endpoint": SINGLE_OPTIONAL_STRING}
...
OPTIONAL_OAUTH_PROTECTED_RESOURCE_METADATA = (
    ...
    False,
    msg_ser,
    oauth_protected_resource_deser,
    False,
)


class OIDCRPMetadata(oidc.RegistrationRequest):
    c_param = oidc.RegistrationRequest.c_param.copy()
    c_param.update({"client_registration_types": REQUIRED_LIST_OF_STRINGS})
...
OPTIONAL_RP_METADATA = (Message, False, msg_ser, rp_metadata_deser, False)


class OIDCRPRegistrationResponse(oidc.RegistrationResponse):
    c_param = oidc.RegistrationResponse.c_param.copy()
    c_param.update({"client_registration_types": REQUIRED_LIST_OF_STRINGS})
...
OPTIONAL_RP_REGISTRATION_RESPONSE = (
    ...
    False,
    msg_ser,
    rp_registration_response_deser,
    False,
)


class OPMetadata(oidc.ProviderConfigurationResponse):
    c_param = oidc.ProviderConfigurationResponse.c_param.copy()
    c_param.update(
        {
...
class FedASConfigurationResponse(oauth2.ASConfigurationResponse):
    c_param = oauth2.ASConfigurationResponse.c_param.copy()
    c_param.update(
        {
            "organization_name": SINGLE_OPTIONAL_STRING,
            "contacts": OPTIONAL_LIST_OF_STRINGS,
...
class Policy(Message):
    ...
    def verify(self, **kwargs) -> Literal[True]:
        ...
        if _extra_parameters:
            ...
            if _critical is None:
                ...
                if _known:
                    ...
                    raise UnknownCriticalExtension(
                        _musts.intersection(_extra_parameters)
                    )
        return True
            ...
            if _trust_mark:
                ...
                if _trust_mark_type:
                    ...
                    if _tm_id != _trust_mark_type:
                        raise ValueError(
                            "The Trust Mark identifier MUST have the same value as the trust_mark_type "
                            "claim"
                        )
                else:
...
class TrustMarkOwners(Message):
    ...
    def verify(self, **kwargs) -> Literal[True]:
        ...
        for owner_id, spec in self.items():
            if (
                "sub" in spec and "jwks" in spec
            ):  # If there are other claims ignore them
                continue
            else:
                if "sub" not in spec:
            ...
            for _tm in _trust_marks:
                ...
                if isinstance(_tm["trust_mark"], str):
                    ...
                    if _payload["trust_mark_type"] != _tm["trust_mark_type"]:
                        raise ValueError("trust_mark_type values does not match")
        ...
        if "trust_mark" not in self:
            if "sub" not in self or "trust_mark_type" not in self:
                raise AttributeError(
                    "Must have both 'sub' and 'trust_mark_type' or 'trust_mark'"
                )
        return True
...
class ListResponse(Message):
    c_param = {"entity_id": REQUIRED_LIST_OF_STRINGS}
...
class RegistrationRequest(oidc.RegistrationRequest):
    c_param = oidc.RegistrationRequest.c_param.copy()
    c_param.update(
        {
...
class RegistrationResponse(oauth2.ResponseMessage):
    """
    Response to client_register registration requests
    """

    c_param = oauth2.ResponseMessage.c_param.copy()
    c_param.update(
        {
            "client_id": SINGLE_REQUIRED_STRING,
            "client_secret": SINGLE_OPTIONAL_STRING,
...
class TrustMarkRequest(Message):
    c_param = {"trust_mark_type": SINGLE_REQUIRED_STRING, "sub": SINGLE_REQUIRED_STRING}
...
class WhoResponse(Message):
    c_param = {"entities_to_use": REQUIRED_LIST_OF_STRINGS}
